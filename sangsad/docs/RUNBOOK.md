...
| `results:wikipedia` | drafts 2026 general-election results for every territorial seat from Wikipedia (Bangla first, then English), read through `/wiki/<title>?action=raw` because robots.txt disallows the API; writes `docs/reports/results-wikipedia-<date>.json` and, with the mymp secrets, inserts drafts into mymp.bd's `election_results` (seats that already have a row are never touched). A winner whose name does not match the seat's sitting member is flagged in the note | MYMP_SUPABASE_URL and MYMP_SUPABASE_SERVICE_ROLE_KEY to write |
