...
**M2, results so far (2026-09-11):** with ecs.gov.bd closed to bots, the owner chose Wikipedia as a draft source. `results:wikipedia` found a usable 2026 general-election result for 144 of 300 seats and wrote them to mymp.bd as drafts with the article link; 137 winners match the sitting member (5 more differ only in spelling; Bogura-6 and Mymensingh-2 are flagged). Editors check each against the ECS gazette before publishing; the result card now names Wikipedia as the source when the link is Wikipedia's. The official terms count (`count` on the term record) and resignations are now shown; see the commit history.
