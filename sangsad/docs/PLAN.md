...
This work is for **mymp.bd**. The original brief placed it under another site's `/sangsad` path; the owner has since confirmed the সংসদ project is for My MP. Decision: **the live mymp.bd stays the only website and admin, and this workspace becomes its data engine.** Everything below replaces the original phases 3–6; phases 1–2 (the database, the nightly worker, 349 members and their photos on Supabase) are done and are kept as they are.

What stays from the brief: plan and approve each phase; never invent data (gaps read "তথ্য পাওয়া যায়নি"); TEST_ fixtures that cannot load in production; inspect every source before ingesting and record it in SOURCES.md; link-only news (headline, source, time, link; no full text, no copied images); one request per second per domain and robots.txt respected; never scrape Facebook; the neutrality rules; matcher thresholds (≥0.85 auto, 0.5–0.85 review, <0.5 discard). What goes: the other site's base path, its name and bot string, the rule about that site's own articles, and the separate Next app in `apps/web` (mymp.bd already is the website).
...
**M0: Rename and clean up (half a day).** Folder renamed to `sangsad/` and packages to `@sangsad/*`; delete `apps/web`; bot string → `MyMPBot/1.0 (+https://mymp.bd/somporke)`; docs, CI and worker workflow renamed; mymp.bd's `ignoreCommand` and `tsconfig` exclude follow the new folder. The workflow reads the `SANGSAD_*` secrets (the owner moved them to the new names and deleted the old ones on 2026-09-11). Optional: rename the Supabase project to "sangsad" in its dashboard. Done when: the old name is gone from the code and docs, CI green, one worker run green.

**M1, as built (2026-09-11):** rather than re-deriving mymp.bd's JSON from the tables (which would have duplicated about 400 lines of its sync in a second language and risked quiet differences on a live site), the engine publishes a cleaned copy of the exact parliament.gov.bd responses mymp.bd's sync reads, and the sync reads that copy instead of the government server. The same code then produces the same output, which the before/after diff proves. Photos come from the engine's storage. The tables stay the base for M2 and M3. Details in RUNBOOK.md, "The mirror mymp.bd builds from".

**M1: mymp.bd reads the engine (about a day), as first planned.** `scripts/sync.mjs` reads members, terms (including earlier terms), committees (roster rule), sessions and notices from the sangsad database and writes the same `data/*.json` shapes, so no page changes. MP photos switch from `prp.parliament.gov.bd` (slow, TLS quirks) to the stored copies. If the database is unreachable the build falls back to today's live-API path. The worker workflow ends by calling mymp.bd's deploy hook, and mymp.bd's own 02:00 cron moves to 03:00 as a backstop. Done when: a mymp.bd build from the database produces the same counts as today (349 members, earlier terms, committees, notices), photos load from storage, and a before/after diff of `data/*.json` is reviewed.

**M2, results published (2026-09-11, on the owner's instruction):** 297 of 300 seats are live on mymp.bd. `results:2026` reads The Business Standard's Election 2026 page (297 seats, every candidate) and Wikipedia (143 seats overlap: the winner's count agrees in 101, differs in 42, and the note gives Wikipedia's count; Sherpur-3 comes from Wikipedia alone). A parser fault that ran Mymensingh-2's 2026 box into the 2024 one was found and fixed first. Held back as a draft: Khagrachhari (298), where TBS's counts add up to 7,33,509 against 5,54,119 voters and name a winner who is not the sitting member. No source has Chattogram-2 or Chattogram-4. None of this is the ECS gazette, and every card says so.

**Earlier (2026-09-11):** with ecs.gov.bd closed to bots, the owner chose Wikipedia as a draft source. `results:wikipedia` found a usable 2026 general-election result for 144 of 300 seats and wrote them to mymp.bd as drafts with the article link; 137 winners match the sitting member (5 more differ only in spelling; Bogura-6 and Mymensingh-2 are flagged). Editors check each against the ECS gazette before publishing; the result card now names Wikipedia as the source when the link is Wikipedia's. The official terms count (`count` on the term record) and resignations are now shown; see the commit history.
...
**M3, first delivery (2026-09-11):** the owner sent the list of 80 outlets (50 Bangladeshi newspapers and portals, 20 TV channel websites, 10 international). `sources:inspect` found a usable RSS feed or Google News sitemap for 43; 21 are blocked (a bot challenge or an unreadable robots.txt, among them bdnews24, Kaler Kantho, Jugantor, Jagonews24, Dhaka Post, Banglanews24, Risingbd, Somoy, Jamuna and AP, and Reuters, whose robots.txt refuses us), and 16 publish no usable feed. Nothing works around a challenge; Google News RSS is not used because its robots.txt disallows it. The matcher needs a member's full name and a context signal (office, seat, "এমপি", ministerial title) and never links a bare two-word name; 9 tricky-case tests. First run: 3,040 headlines from 7 days, 40 automatic matches (all 40 checked by hand, all correct), 114 for review. mymp.bd now shows automatic matches as published news and holds the rest as drafts in its existing news admin; the News page copy was changed to say so. Still to do in M3: YouTube uploads, the 48-hour soak, and a review screen that shows the match reason.

**M3: Automatic news per MP (brief phase 4), as first planned.** Inspect the 50 sources in `config/sources.json`, RSS every 30 minutes, Google News RSS as a rate-limited fallback, YouTube uploads playlists; the matcher (alias table already seeded, context signals, ambiguity guard, `match_reason`) links items to MPs; 0.5–0.85 matches wait in a review queue in mymp admin. Items appear on MP pages and `/songbad` next to the manual news posts, link-only. 48-hour soak and a 100-item accuracy check before it goes public.
...
# History: the original plan (phases 1–2 done, kept for the record)

Public, bilingual (বাংলা first) directory of every Member of the 13th Jatiya Sangsad, with a verified profile and an auto-updated, link-only news feed per member. Originally planned for another site's `/sangsad` path; now the data engine behind mymp.bd (see the top of this file).
...
| parliament.gov.bd TLS | Server **omits its intermediate certificate** (GoGetSSL) and **resets connections without a User-Agent**. Plain `fetch()` in Node fails. Fix: `node:https` with the intermediate + root added alongside Node's bundled CAs, and a UA string. | Done in `@sangsad/shared`; the seed already uses it. |
...
```
sangsad/                   inside sumongetup/mymp
...
../.github/workflows/      sangsad-ci.yml at the repository root (path-filtered); worker schedule from Phase 2
```
...
7. **Schedule.** `.github/workflows/sangsad-worker.yml` runs `parliament` nightly (02:00 Dhaka) with `DATABASE_URL` and the service key as repository secrets; also runnable by hand.
